...
path <- "resOrdinalSimulations.txt"
...
for(T in tabT){
	...
	for(r in 1:reps){
		#Generate time series with BAR(1) rank counts
		X <- rbinom(1, parN,parp)
		
		data <- rep(NA, T)
		for(t in 1:T){
			X <- rbinom(1, X,alph)+rbinom(1, parN-X,bet)
			data[t] <- X
		} #for t
